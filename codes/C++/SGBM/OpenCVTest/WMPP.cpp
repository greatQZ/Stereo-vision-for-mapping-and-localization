...
namespace WMPP_FUNC{
	...
	void saveChk( const int hei, const int wid,  int* lValid, int* rValid )
	{
		...
		cv::imwrite( "l_chk.png", lChk );
		cv::imwrite( "r_chk.png", rChk );
	}
}

void WMPP::postProcess( const Mat& lImg, const Mat& rImg, const int maxDis, Mat& lDis, Mat& rDis, Mat& lSeg, Mat& lChk )
{
    ...
    for( int i = 0; i < 3; i ++ ) {
		// save check results
		WMPP_FUNC::lrCheck( lDis, rDis, lValid, rValid );
		cout<<"lrcheck finish"<<endl;
		WMPP_FUNC::fillInv( lDis, rDis, lValid, rValid );
		cout<<"fillInv finish"<<endl;
		WMPP_FUNC::wgtMedian( lTmp, rTmp, lDis, rDis, lValid, rValid, maxDis);
	}
